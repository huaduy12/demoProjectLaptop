...
public class searchHeraderServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		productDao productDao = new productDao();
	       ...
	       String namelaptop = request.getParameter("namelaptop");
	       
	       List<Product> products;
	       if(Integer.parseInt(cate_id) != 0) {

	        products = productDao.serachProduct(Integer.parseInt(cate_id), namelaptop);

	       }else {
	    	products = productDao.serachProductByName(namelaptop);
